...
export class HttpService {
  ...
  async get(url: string): Promise<Response> {
    try {
      ...
      let result = await this.http.get<any>(environment.baseUrl + url, { observe: 'response', headers: headers }).toPromise();
      return { body: result.body, status: result.status, statusText: result.statusText };
    } catch (error) {
  ...
  async delete(url: string): Promise<Response> {
    try {
      let headers: HttpHeaders = this.getHeaders();
      let result = await this.http.delete<any>(environment.baseUrl + url, { observe: 'response', headers: headers }).toPromise();
      return { body: result.body, status: result.status, statusText: result.statusText };
    } catch (error) {
      return Promise.reject(this.handleError(error));
    }
